...
catchup --repo . --since 30d
```

For a checkout without installing:

```sh
PYTHONPATH=src python3 -m catchup --repo . --since 30d
```
...
On later sessions, use `--since last` to start at the previous successful run. Catchup defaults to five focused items, infers the identity from `git config user.email`, and accepts `--me` when a repository uses multiple addresses.
...
The research behind the personal re-entry direction and the next product
iterations is in [`docs/field-research.md`](docs/field-research.md).
